...
def _in_projection_packed(
    ...
    if k is v:
        if q is k:
            ...
            kv_proj = functional.linear(k, w_kv, b_kv)
...
def multi_head_attention_forward(
    ...
    if need_weights:
        ...
        if exe_ids is not None:
            exe_ids2 = exe_ids.clone().permute([1,0]).view(bsz, 1, src_len).expand(bsz, num_heads, src_len).unsqueeze(-1).expand(bsz, num_heads, src_len, src_len).clone()
            exe_ids3 = exe_ids.clone().permute([1,0]).view(bsz, 1, src_len).expand(bsz, num_heads, src_len).unsqueeze(-2).expand(bsz, num_heads, src_len, src_len).clone()
            
            exe_ids3[:,4:8] = 0
            exe_ids2[:,4:8] = 0

        if graph_score is not None and exe_ids is not None:
            gate_value = gate_value.view(bsz, 1, 1).expand(bsz, num_heads, 1).view(bsz, num_heads, 1, 1).expand(bsz, num_heads, 1, src_len)
            graph_score = graph_score.view(bsz, 1, 1, src_len).expand(bsz, num_heads, 1, src_len)
            graph_score_2 = (torch.nn.Softmax(dim=-1)(attn_mask + graph_score)) * gate_value
            diagonal_matrix = torch.eye(src_len).to(device)
